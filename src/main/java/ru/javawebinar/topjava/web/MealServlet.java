...
import ru.javawebinar.topjava.util.InMemoryDaoMeal;
...
//        response.sendRedirect("meals.jsp");
        ...
        String forward = "";
        mealsTo = MealsUtil.filteredList
                (dao.getAllMeals(), LocalTime.of(0, 0), LocalTime.of(23, 59), CALORIES_LIMIT);
        if (action == null) {
            forward = MEALS_LIST;
...
//            request.setAttribute("meals", mealsTo);
    ...
    protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
        ...
        if (id == null || id.isEmpty()) {
            dao.addMeal(dateTimeMeal, description, calories);
        } else {
            dao.updateMeal(new Meal(Integer.parseInt(id), dateTimeMeal, description, calories));
        }
        response.sendRedirect("meals");
